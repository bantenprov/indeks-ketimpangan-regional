...
export default {
  mounted() {
    axios.get('api/ketimpangan-regional/' + this.$route.params.id + '/edit')
      .then(response => {
        if (response.data.status == true) {
          this.model.label = response.data.ketimpangan_regional.label;
          this.model.old_label = response.data.ketimpangan_regional.label;
          this.model.description = response.data.ketimpangan_regional.description;
        } else {
          alert('Failed');
        }
      })
      .catch(function(response) {
        alert('Break');
        window.location.href = '#/admin/ketimpangan-regional';
      });
  },
  data() {
    return {
  ...
  methods: {
    onSubmit: function() {
      ...
      if (this.state.$invalid) {
        ...
        axios.put('api/ketimpangan-regional/' + this.$route.params.id, {
    ...
    back() {
      window.location = '#/admin/ketimpangan-regional';
    }
  }
}
</script>
